public class MinMax {
    ...
    public void insert_node(int data) {    //  addNode() will add a node to the list
        ...
        if (head == null) {                // If list is empty  
            head = newNode;         //Both head and tail will point to newNode  
            head.prev = head.next = null;  //head's & tail's previous will point to null  
        }
        else {  
            head.next = newNode;         //newNode will be added after tail such that tail's next will point to newNode
            newNode.prev = head;         //newNode's previous will point to tail   
            head = newNode;              //newNode will become new tail
            head.next = null;            //As it is last node, tail's next will point to null  
        }
    }
